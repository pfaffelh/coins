...
Even a bit more can be said for strategy $A$: Let $X_1, X_2,...$ be
the (random) total number of heads which are set aside when throwing
$1,2,...$ coins and following strategy $A$. If we throw $n$ coins and
$1\leq X_n < n$, then $X_n = 1 + X_{n-1}$ etc. We use that, for
$\mathcal F_{n-1} = \sigma(X_1,...,X_{n-1})$, and with $Z_n := n -
X_n$, with a similar calculation as above,
\begin{align*}
  \mathbb E[Z_n | \mathcal F_{n-1}] & = (Z_{n-1} + 1)(1-p)^n + (1 - p^n - (1-p)^n) Z_{n-1}
  \\ & = (1-p)^n + (1-p^n) Z_{n-1}.
\end{align*}
In this situation, we can use the following martingale\footnote{Recall
  that $(Y_n)_{n=1,2,...}$ is a martingale iff $\mathbb E[Y_n |
    \mathcal F_{n-1}] = Y_{n-1}$.}:

\begin{lemma}
  Let $Z_0 = 0, Z_1,...$ and $\mathcal F_{n-1} :=
  \sigma(Z_1,...,Z_n)$. If $\mathbb E[Z_n | \mathcal F_{n-1}] = a_n +
  b_n X_{n-1}$ for all $n = 1,2,...$, then with $c_n = b_n c_{n-1} + 
  a_n$,
  $$ \frac{Z_n - c_n}{b_1 \cdots b_n} \text{ is a martingale}.$$ Note
  that the recursion for $c_n$ is solved by 
  \begin{align*}
    c_0=0 \text{ and }c_n & = \sum_{k=1}^n b_n \cdots b_{k+1} a_k.
  \end{align*}
  In particular,
  \begin{align*}
    \mathbb E[Z_n] & = b_1 \cdots b_n \frac{\mathbb E[Z_n] - c_n}{b_1
      \cdots b_n} + c_n = b_1 \cdots b_n (\mathbb E[Z_0] - c_0) + c_n
    = c_n = \sum_{k=1}^n b_n \cdots b_{k+1} a_k.
  \end{align*}
\end{lemma}
...
\begin{proof}
  We write directly
  \begin{align*}
    \mathbb E\Big[ \frac{Z_n - c_n}{b_1 \cdots b_n} \Big| \mathcal
      F_{n-1}\Big] & = \frac{a_n + b_nX_{n-1} - b_nc_{n-1} - a_n}{b_1
      \cdots b_n} = \frac{X_{n-1} - c_{n-1}}{b_1
      \cdots b_{n-1}},
  \end{align*}
  which proves the martingale property. For the recursion formula, we
  note that
  \begin{align*}
    c_n & = b_n \Big(\sum_{k=1}^{n-1} b_{n-1} \cdots b_{k+1} a_k\Big) +
    a_n,
  \end{align*}
  and we are done.
\end{proof}
...
So, we have that
$$ \frac{Z_n - \sum_{k=1}^n (1-p^n)\cdots
  (1-p^{k+1})(1-p)^k}{(1-p)\cdots (1-p^n)}$$ is a martingale. Since
the denominator is bounded away from $0$ for all $p$, and the sum in
the numerator converges as $n\to\infty$, we find that the martingale
is bounded from below, and hence converges almost surely. 

\newpage

Strategy $C$: Always set aside as many heads as possible: Starting
with $n$ coins and with $k\leq n$, let $p_n(k)$ be the probability
that there are $k$ coins left at some step. Then, we get the recursion
$$ p_n(k) = p_n(k+1) (1-p)^{k+1} + \sum_{\ell = k+1}^n p_n(\ell)
\binom{\ell}{k} p^k (1-p)^{\ell-k}.$$ Hence, with $\psi_n(s) :=
\sum_{k=0}^\infty p_n(k) s^k$ and $s\in (0,1)$,
\begin{align*}
  \psi_n(s) & = \sum_{k=0}^\infty p_n(k) s^k = \frac{1}{s}
  \sum_{k=0}^\infty p_n(k+1)(s(1-p))^{k+1} + \sum_{\ell=1}^n p_n(\ell)
  \sum_{k=0}^{\ell-1} \binom{\ell}{k} p^k (1-p)^{\ell-k} s^k \\ & =
  \frac{\psi_n(s(1-p)) - 1}{s} + \psi_n(1 - p + ps) - \psi_n(ps),
\end{align*}
where we have used that $p_n(0)=1$ (since the experiment certainly
ends with~0 coins).
...
Let us study the case $p<1/2$. Setting $w_{0,p} := 1$, note that
\eqref{eq:wn} changes to
\begin{align}\label{eq:wn1}
  w_{n,p} & = \sum_{j=0}^{n-1} \binom n j p^{n-j} (1-p)^j \max_{j\leq
    i\leq n-1} w_{i,p} \\ & = p \sum_{j=0}^{n-1} \binom{n-1}{j}
  p^{n-1-j}(1-p)^j w_j + \sum_{j=0}^{n-2} \binom{n-1}{j}
  p^{n-1-j}(1-p)^{j+1} w_{j+1} \\ & = (1-p)^{n-1}p w_{n-1} +
  \sum_{j=0}^{n-2} \binom{n-1}{j} p^{n-1-j}(1-p)^j ((1-p)w_{j+1} +
  pw_j)
\end{align}
Let $p = \tfrac 12 - \varepsilon$ and $w_n = \tfrac 12 -
\delta_n$. Then,
\begin{align}\label{eq:wn1}
  \delta_{n} & = \tfrac 12 - \sum_{j=0}^{n-1} \binom n j p^{n-j}
  (1-p)^j (\tfrac 12 -\delta_j) \\ & = \tfrac 12 \big( \tfrac 12 +
  \varepsilon\big)^n - \tfrac 12 (\tfrac 12 -\varepsilon)^n +
  \sum_{j=1}^{n-1} \binom{n}{j} (\tfrac 12 - \varepsilon)^{n-j}
  (\tfrac 12 + \varepsilon)^j \delta_j 
\end{align}
...
\begin{align*}
  w_2 - w_3 & = 3p(1-p)^2 (w_1 - w_2), \\
  w_2 - w_3 & = -p(1-p)^2 w_2 + p^2(1-p)(1-w_1) + 2 p (1-p)^2 (w_1 - w_2)\\
  w_1 - w_2 & = p(1 - 3p + 2p^2) = p(1-2p)(1-p) = p(1-2p)(w_0 - w_1)
\end{align*}
$$ w_2 - w_3 = 3p(1-p)^2 (w_1 - w_2)$$
...
\begin{align*}
  ...
  w_{j+1,p}) \\ & = - p^n + \sum_{k=0}^{n-2} p^{n-k}(1-p)^k w_k -
...
Let us study the latter, which holds for $p<1/2$ and $n=2, 3$. So, we
have shown that for $p<1/2$, we have $w_{1,p} > w_{2,p} > w_{3,p}$.

If $w_{k,p} \leq \frac{p^{k-1}}{(1-p)^{k-2}}$ for all $k\leq n-1$,
then
\begin{align*}
  w_{n,p} & = p^n + \sum_{j=1}^{n-1} \binom n j p^{n-j} (1-p)^j
  w_{j,p} \\ & \leq p^n + \sum_{j=1}^{n-1} \binom n j p^{n-j} (1-p)^j
  \frac{p^{j-1}}{(1-p)^{j-2}}
  \\ & = p^n + \sum_{j=1}^{n-1} \binom n j p^{n-1} (1-p)^2
  \\ & = p^{n-1}(p + (1-p)^2(2^n - 2)
\end{align*}
so $w_{n-1,p} < \frac{p^{n-1}}{(1-p)^{n-2}}$ if $(1-p)^{n-2}(p +
2(1-p)^2(2^{n-1}-1)) < 1$.


Furthermore,
$$ w_{4,p} = p^4 + 4p^3(1-p) w_{1,p} $$


,
which is certainly the case if $w_{n-2,p} <
\frac{p^{n-2}}{(1-p)^{n-3}}$ or $w_k \leq \frac{p^k}{(1-p)^{k-1}}$ for
all $k$.

$w_{1,p} > w_{2,p}$ if $w_{1,p} = p < 1-p$.

$w_{2,p} > w_{3,p}$ if $w_{2,p} < p = w_{1,p}$
