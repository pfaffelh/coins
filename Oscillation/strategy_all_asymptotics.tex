...
\textbf{Status note (2026-05).} Sections~\ref{sec:saddle}
(saddle-point) and~\ref{sec:mellin} (Mellin view) below are
\textbf{superseded} by \S3 of \texttt{manuscript.tex}, which derives
the log-periodic asymptotics rigorously and in full by an exact
product identity plus analytic de-Poissonisation. They are kept only
as a record of the exploratory route, and \S\ref{sec:mellin} contains
errors: the harmonic sum $\sum_{k\ge0}\log(1-e^{-pq^kz})$ diverges
(the convergent sum runs over $q^{-j}$, not $q^{k}$); the fundamental
strip of $\mathcal M[\log(1-e^{-pz})]$ is $\Re s>0$, not
$-1<\Re s<0$; and $\log B(z)$ carries \emph{no} $\log z$ term, since
$b_n\le1$ forces $B(r)\le e^{r}$. The corrected analysis is in the
manuscript. The functional equation (\S\ref{sec:setup}) and the
numerics (\S\ref{sec:numerics}) stand unchanged.}}
\medskip
...
\end{equation}
(We used $\prod_k e^{pq^kz}=e^{pz\sum_k q^k}$ and
$p\sum_{k=0}^{M-1}q^k = 1-q^M$, so the exponent is $pz\sum_k q^k=z(1-q^M)$.)
On the
...
\approx e^{z(1-q^M)}$ which is monotone increasing in~$M$; for $M>K(z)$
...
\begin{equation}\label{eq:Basymp}
  B(z) \;=\; e^{z}\,\Phi\!\bigl(\log z\bigr)\,\bigl(1+o(1)\bigr),
  \qquad z\to+\infty,
\end{equation}
...
\end{equation}
Using $B(u)\sim e^{u}\Phi(\log u)$ gives $u B'/B\sim u$ and so
$u_n\sim n$ and $\beta_n\sim n$. Substituting
$B(u_n)\sim e^{u_n}\Phi(\log u_n)\sim e^{n}\Phi(\log n)$:
\begin{align*}
  b_n &\sim \frac{n!\,e^{\,n}\,\Phi(\log n)}{n^n\,\sqrt{2\pi n}}
       \;\sim\; \frac{n^n e^{-n}\sqrt{2\pi n}\;\,e^n\,\Phi(\log n)}
                     {n^n\,\sqrt{2\pi n}} \\[-0.2em]
  &\mkern10mu \textrm{(Stirling $n!\sim n^ne^{-n}\sqrt{2\pi n}$; the
    $e^{\pm n}$, $n^n$ and $\sqrt{n}$ cancel)}\\[0.4em]
  &\mkern70mu \boxed{\;b(p,n) \;=\; \Phi_p\bigl(\log n\bigr)\,\bigl(1+o(1)\bigr).\;}
\end{align*}
...
$0\le b_n\le 1$ forces $B(r)\le e^{r}$ on the positive axis, so $B$
grows at the same rate as $e^{z}$, not faster --- but the bounded,
non-constant log-periodic factor $\Phi$ is exactly the feature that
the closure theory of H-admissible functions does not cover.
(b) Justification of the contour shift in the Mellin step, i.e.\
establishing the strip of analyticity for the Mellin transform of
$\log B(z)-z$. Both are standard in spirit (Flajolet et al.) but
non-trivial in detail. I have not attempted a rigorous write-up here;
the note is intended as a map.
...
3^k$ keeps the phase $\{\log n/\log(1/q)\}$ \emph{exactly} constant
...
$\Phi_p(\log n)$ of period $\log\bigl(1/(1-p)\bigr)$, with
