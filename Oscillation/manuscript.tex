...
arithmetic heart of the proof. By the decomposition above it needs
two things. \emph{(i)} Pure strategy~\All{} oscillates:
$A_\infty(b)\ne0$, equivalently $\Phi_p$ is non-constant
(Section~\ref{sec:nonconst}), equivalently
$\mathcal M[\Xi](2\pi im/L)\ne0$ for some $m\ne0$. \emph{(ii)} The
perturbation $g$ must not cancel it: $A_\infty(g)\ne-A_\infty(b)$.
Positivity of $g$ alone does \emph{not} secure~(ii). The cone
$\mathcal C$ is \emph{not} pointed: its generators $A_\infty(e^{(r)})$
--- the windowed limits of the basis solutions started from
$\delta_r$ --- wind around the origin, the recursion being not
scale-invariant (verified numerically,
\texttt{simulation/d2\_cone.py}); so some non-negative solution
\emph{does} satisfy $A_\infty(g)=-A_\infty(b)$. Excluding this for the
\emph{optimal} $x=w$ must use the optimality of $w$, not merely
$w\ge b$. Work continues here.}
...
A further computation (\texttt{simulation/d2\_cone.py}) shows the cone
$\mathcal C$ of Section~\ref{sec:base} is \emph{not} pointed: the
windowed limits $A_\infty(e^{(r)})$ of the basis solutions wind around
the origin (angular span $>\pi$). This rules out a positivity-only
proof of Gap~D.
...
A positivity-only reduction (the cone $\mathcal C$ pointed) is ruled
out --- $\mathcal C$ is not pointed (\texttt{simulation/d2\_cone.py})
--- so Gap~D must exploit the optimality of $w$; that is the current
front of work.
