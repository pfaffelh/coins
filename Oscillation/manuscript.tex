...
\noindent\emph{The residual $\rho$: a Taylor expansion in the scale.}
The kernel difference $\delta_s:=\nu^{(s+L)}-\nu^{(t+L)}$ defining
$\rho$ vanishes at the window centre, $\delta_t=0$. Expanding it in
$s$ about $s=t$,
\begin{equation}\label{eq:deltataylor}
  \delta_s=(s-t)\,\dot\nu+O\bigl((s-t)^2\bigr),
  \qquad \dot\nu:=\partial_\tau\nu^{(\tau)}\big|_{\tau=t+L},
\end{equation}
where $\dot\nu$ is a \emph{fixed} signed measure of total mass zero
(every $\nu^{(\tau)}$ is a probability measure). The fixed mass-zero
measure brings the windowed-projection identities back into play.

\begin{lemma}[leading order of the residual]\label{lem:rho1}
  Suppose the kernel family is scale-regular:
  \begin{equation}\label{eq:scalereg}
    \partial_\tau\hat\nu^{(\tau)}(-\omega)=O(e^{-\tau}),
    \qquad
    \int|\zeta|\,\bigl|\partial_\tau\nu^{(\tau)}\bigr|(d\zeta)
      =O(e^{-\tau/2}).
  \end{equation}
  Then the leading Taylor term of the residual,
  \[
    \rho_1(t):=\frac1L\int e^{-i\omega s}(s-t)\,
      \chi\!\bigl(\tfrac{s-t}L\bigr)
      \Bigl(\textstyle\int y(s+\zeta)\,\dot\nu(d\zeta)\Bigr)\,ds,
  \]
  satisfies $\rho_1(t)=O(e^{-t/2})$.
\end{lemma}

\begin{proof}
  Write $(s-t)\chi(\tfrac{s-t}L)=L\,\psi(\tfrac{s-t}L)$ with
  $\psi(u):=u\,\chi(u)\in C_c^\infty$. By Fubini and the translation
  substitution of Lemma~\ref{lem:wsi}(b) --- whose proof is valid for
  any window and any finite signed measure ---
  \[
    \rho_1(t)=L\int e^{i\omega\zeta}\,A^{[\psi]}(t+\zeta)\,
             \dot\nu(d\zeta),
  \]
  where $A^{[\psi]}$ is the windowed coefficient with window $\psi$,
  again $C^\infty$ with bounded derivatives. As $\dot\nu$ has mass
  zero,
  \[
    \rho_1(t)=L\,A^{[\psi]}(t)\,\widehat{\dot\nu}(-\omega)
      +L\int e^{i\omega\zeta}
        \bigl(A^{[\psi]}(t+\zeta)-A^{[\psi]}(t)\bigr)\,\dot\nu(d\zeta).
  \]
  In the first term $\widehat{\dot\nu}(-\omega)
  =\partial_\tau\hat\nu^{(\tau)}(-\omega)\big|_{t+L}=O(e^{-t})$ by the
  first bound of~\eqref{eq:scalereg}, and $A^{[\psi]}$ is bounded. In
  the second, $|A^{[\psi]}(t+\zeta)-A^{[\psi]}(t)|\le C|\zeta|$, so it
  is $O\bigl(\int|\zeta|\,|\dot\nu|(d\zeta)\bigr)=O(e^{-t/2})$ by the
  second bound. Hence $\rho_1(t)=O(e^{-t})+O(e^{-t/2})=O(e^{-t/2})$.
\end{proof}

\gap{Gap C --- scale-regularity of the kernel}{Proposition~\ref{prop:prop}
is now reduced to two precisely delimited items. (i) The
scale-regularity bounds~\eqref{eq:scalereg}: a characteristic-function
estimate and a first-moment estimate for the \emph{scale-derivative}
of the binomial log-deviation law --- a differentiated Edgeworth
expansion for $\mathrm{Bin}(n,q)$, standard in spirit (Petrov-type
local limit theory). (ii) The higher Taylor orders $O((s-t)^2)$
in~\eqref{eq:deltataylor}, handled by the same device with windows
$u^k\chi(u)$ and a routine remainder estimate (the window has compact
support). Neither is carried out here; this is where work continues.}
...
then D. Within Gap~C the leading order is settled
(Lemma~\ref{lem:rho1}): the remainder is now reduced to the
scale-regularity bounds~\eqref{eq:scalereg} --- a differentiated
Edgeworth expansion for the binomial --- together with the routine
higher Taylor orders. That is the current front of work.
