...
constant, equivalently $\mathcal M[\Xi](2\pi im/L)=0$ for every
$m\ne0$. At $p=\tfrac12$ one has $b_n\equiv\tfrac12$ and
$\Phi_p\equiv\tfrac12$. For $p<\tfrac12$ the non-constancy of $\Phi_p$
--- that $b_{n,p}$ genuinely oscillates --- is the arithmetic heart of
the paper; it is Gap~D (Section~\ref{sec:base}).

\begin{remark}[the fair point]
  Near $p=\tfrac12$ the non-constancy is explicit. With
  $p=\tfrac12-\varepsilon$ and $b_{n,p}=\tfrac12-\varepsilon\beta_n
  +O(\varepsilon^2)$, the first-order perturbation has generating
  function $\mathcal B(z)=z\,e^{z/2}+(e^{z/2}-1)\,\mathcal B(z/2)$;
  the same product identity gives its profile Fourier coefficients
  $-L^{-1}\Gamma(1+s_m)\,D(1+s_m)$, with $s_m=2\pi im/L$, $L=\log2$,
  and $D(\sigma)=\sum_{m\ge1}(-1)^{s_2(m)}m^{-\sigma}$ the
  \emph{Thue--Morse Dirichlet series} ($s_2$ the binary digit-sum).
  Since $D(1+2\pi i/L)=-2.320-0.313\,i\ne0$ (numerically), the
  first-order perturbation oscillates, and $\Phi_p$ is non-constant
  for $p$ slightly below $\tfrac12$.
\end{remark}
...
arithmetic heart of the proof. It needs two things: that pure
strategy~\All{} oscillates --- $A_\infty(b)\ne0$, the non-constancy of
$\Phi_p$ (Section~\ref{sec:nonconst}) --- and that the perturbation
$g$ does not cancel that oscillation, $A_\infty(g)\ne-A_\infty(b)$.
The first is settled near $p=\tfrac12$ (Section~\ref{sec:nonconst})
and expected throughout $(0,\tfrac12)$; the second is not secured by
$g\ge0$ alone, and must draw on the optimality of $w$. Work continues
here.}
...
The non-constancy of $\Phi_p$ near $p=\tfrac12$ is confirmed
separately (\texttt{simulation/d1\_perturbation.py},
\texttt{d1\_thue\_morse.py}): the first-order perturbation
oscillates, and the Thue--Morse value $D(1+2\pi i/L)$ is non-zero.
...
A single gap remains open; it is flagged in the text and summarised
here.
...
is the non-degeneracy $A_\infty(x)\ne0$ (Gap~D), the arithmetic
heart: that strategy~\All{} oscillates --- settled near $p=\tfrac12$
(Section~\ref{sec:nonconst}) --- and that the optimal perturbation
does not cancel that oscillation. This is the current front of work.
