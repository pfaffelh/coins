...
GAP} \emph{\small markers (Gaps~A, B and~D); Section~\ref{sec:status}
...
(Proposition~\ref{prop:prop}) --- proved in full through the
transposed negative-binomial kernel (Lemma~\ref{lem:transposed}) ---
and the renormalisation theorem (Theorem~\ref{thm:renorm}). Three
steps remain open; they are flagged in the text and summarised here.
...
Proposition~\ref{prop:A} (done) together with Gap~D; Gaps~A and~B
underpin Gap~D and the quantitative form of Theorem~\ref{thm:ALL}.
The former Gap~C --- the propagation remainder --- is now closed: the
transposed kernel (Lemma~\ref{lem:transposed}) reduces it to the exact
negative-binomial identity $\sum_{n\ge j}P(\mathrm{Bin}(n,q)=j)=1/q$,
so that no local limit theorem is needed. The natural order of attack
on what remains is A and~B, then~D; the non-degeneracy $A_\infty\ne0$
(Gap~D) is the current front of work.
